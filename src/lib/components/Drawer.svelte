        ...
        {#each events as event}
          <button on:click={() => goto(`/admin/dashboard?eventId=${event.id}`)}>
            <Item
              id="event-title"
              class={twMerge(
                'w-60 line-clamp-1 md:w-full flex p-2 cursor-pointer bg-gray-100 text-gray-500/80  ease-in-out duration-200 rounded font-semibold',
                event.id === $selectedEventId && 'bg-gray-600 text-gray-100 font-semibold'
              )}>
